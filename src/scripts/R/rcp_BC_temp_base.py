...
pwd = os.getcwd()
rscript_path = os.path.join(pwd, "src/R/rcp_BCtemp_base.R")
...
getOutFilePath = lambda variable: f"/home/abhi/Documents/mygit/BC/output/{variable}"+\
                                      f"/mw30-15/base"
...
for method in ["eqm",
    ...
    if not Path(f'/home/abhi/Documents/mygit/BC/output/{variable}/mw30-15/base/{method}').exists():
        zippedList = list(zip(distFileDict['IMD'],
                              distFileDict['RAW'],
                              ))

        for imdfile, rawfile in zippedList:
            outfile = os.path.join(getOutFilePath(variable),
                                   f"base{imdfile[-8:-3]}")

            os.system(f"Rscript {rscript_path} {method} "+\
                      f"{imdfile} {rawfile} {variable} {outfile}")
        ...
        outfile_parts = f"{getOutFilePath(variable)}/base*"
        outfile_merged = os.path.join(getOutFilePath(variable), method)
